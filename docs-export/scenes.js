    ...
    ff.on("close", () => {
      ...
      if (_skipFrameFast && times.length < MIN_CUTS_FOR_QUALITY) {
        console.log(
          ...
          `retrying with full decode at ${Math.min(maxAnalyzeWidth, 120)}px…`
        );
        detectSceneCuts(filePath, {
          threshold,
          maxAnalyzeWidth: Math.min(maxAnalyzeWidth, 120),
          durationSec,
          onProgress,
...
export function sceneFrameTimes(sc, duration) {
  const scStart = Math.max(0, Number(sc.sceneStart));
  const scEnd = Math.min(Number(sc.sceneEnd), duration - 0.05);
  const range = Math.max(0, scEnd - scStart);
  const cap = duration - 0.1;

  if (range < 0.5) {
    // Very short scene — all 5 frames collapse to the midpoint.
    const mid = Math.min(scStart + range / 2, cap);
    return new Array(FRAMES_PER_SCENE).fill(mid);
  }

  // 5 evenly-spaced positions: 0%, 25%, 50%, 75%, 100% of the scene range.
  return [0, 0.25, 0.50, 0.75, 1.0].map((frac) =>
    Math.min(scStart + range * frac, cap)
  );
}
...
export async function analyzeScenes(filePath, {
  ...
  return { duration, scenes: finalScenes };
}
