/**
 * Pure, self-contained helpers for building/deduping per-beat visual candidates
 * and matching a beat's narration to a Whisper transcript window.
 *
 * Extracted from index.js (v2.9.6) as the first step of modularizing the render
 * worker. These functions have no external/closure dependencies, so moving them
 * here is behaviour-preserving. Keeping them in their own module means a stray
 * edit to index.js can no longer delete them and silently collapse the body
 * (the "_dedupeCandidates is not defined" class of regression) — and the
 * no-undef lint gate now enforces that index.js imports them.
 */

export function _dedupeCandidates(cands) {
  const out = [];
...
export function findTranscriptCandidateForBeat(narration, transcriptSegments, sourceDurationSec = 0) {
