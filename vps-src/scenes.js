...
export function planScenes({
  ...
  // 3) Fallback: too few scenes -> evenly spaced segments.
  const minCount = Math.max(8, Math.round(target * 0.5));
  if (scenes.length < minCount) {
    // FIX (SYNC-FIXES #2b): this fallback previously fired silently, producing
    // a uniform grid with no relationship to real cuts (invisible in logs).
    // Warn loudly so operators can diagnose under-detection instead of seeing
    // an unexplained "no visual match" complaint downstream.
    console.warn(
      `[planScenes] SCENE DETECTION FALLBACK: only ${scenes.length} real scenes ` +
      `(< ${minCount}) — using an even ${(usableEnd / target).toFixed(1)}s grid. ` +
    ...
    // FIX (2026-07-03): flag this render as fallback-grid so downstream
    // render-time logic (the soft logo floor in index.js) knows window[0]
    // is a content-agnostic time-slice, not a real detected scene boundary,
    // and may still contain the opening studio logo/production card.
...
/**
 * Format seconds as mm:ss for burning into frames so Claude sees exact timestamps.
 * e.g. 1245.3 → "20:45"
 */
function fmtSec(t) {
  const s = Math.round(Math.max(0, t));
 ...
 * When `label` is provided (e.g. "20:45"), the timestamp is burned into
 * the top-left corner with FFmpeg drawtext so Claude knows the exact second.
 */
export function extractKeyFrame(filePath, tSec, outPath, maxWidth = 512, label = null) {
  return new Promise((resolve, reject) => {
    // Escape colons for FFmpeg drawtext (colon is a key-value separator in filter syntax).
    const safeLabel = label ? label.replace(/:/g, "\\:") : null;
    const vf = safeLabel
      ? `scale=${maxWidth}:-2,drawtext=text='${safeLabel}':fontsize=20:fontcolor=white:box=1:boxcolor=black@0.65:boxborderw=4:x=8:y=8`
      : `scale=${maxWidth}:-2`;
    const args = [
      "-y",
      "-hide_banner",
...
export async function analyzeScenes(filePath, {
  ...
  const { cuts, maxObservedSec } = await detectSceneCuts(filePath, {
    threshold,
    durationSec: probedDuration,
    // Map the scan's 0..1 fraction onto the 10..33% band of the analyze job
    // so the user sees a moving "Scanning scenes mm:ss / total" indicator.
    onProgress: (frac, msg) => {
      ...
      batch.map(async (sc) => {
        ...
        await Promise.all(
          frameTimes.map(async (tSec, fi) => {
            ...
            try {
              // Fix B: burn the actual timestamp into the frame so Claude cannot
              // misassign footage to the wrong moment based on film memory.
              await extractKeyFrame(filePath, tSec, fp, keyframeWidth, fmtSec(tSec));
              framePaths[fi] = fp;
            } catch {
