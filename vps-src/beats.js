...
export function buildSyncedTimeline(scenes, beatDurations, opts = {}) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const sceneEnd = Number(workScenes[i].endSec);
    // FIX (SYNC-FIXES #7): a CLIP-relocated window is [center - winHalf, center +
    // winHalf] with winHalf >= 8s. Always jumping to the window START meant that
    // when narration is shorter than winHalf (common for short/importance-cut
    // beats), the rendered footage was only [center-8, center-8+need] — the
    // matched moment at `center` itself was never actually shown. If the CLIP/
    // TEXT-MATCH pass stamped a focusSec, center the take on it (clamped inside
    // the window) instead of just jumping to the start.
    const focus = Number(workScenes[i].focusSec);
    ...
    while (need >= minSegSec) {
      ...
      // GLOBAL ceiling (end of the whole source video), never to this beat's
      // own (possibly coverage-expanded) sceneEnd. When a beat's window is
      // narrower than its narration — now common with GSPAN's precise, often
      // short windows — the loop silently kept consuming footage PAST
      // sceneEnd into whatever scene comes next chronologically, producing
      // completely unrelated footage (e.g. "entering a room" narration
      // showing a car driving because the next scene's footage bled in).
      // Clamp to sceneEnd too; when that's the binding constraint (not the
      // true end-of-source), stop for this beat instead of overflowing.
      ...
      if (taken >= minSegSec) {
        ...
        // This beat's own window is exhausted before its narration is — a
        // per-beat footage shortfall, not whole-movie exhaustion. Stop taking
        // footage for this beat rather than bleeding into the next scene's
        // unrelated content; the mux stage's tpad clone-frame tail freezes
        // the last frame for the remaining audio, which is always closer to
        // correct than showing the wrong scene.
        console.warn(`[buildSyncedTimeline] beat ${i}: window exhausted ${need.toFixed(1)}s short of narration — holding last frame (tpad) instead of overflowing into next scene`);
        need = 0;
        break;
      } else {
