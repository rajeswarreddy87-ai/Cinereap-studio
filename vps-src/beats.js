...
export function buildSyncedTimeline(scenes, beatDurations, opts = {}) {
  ...
  const workScenes = scenes.map((sc, i) => {
    ...
    // math below doesn't apply (there's no single "endSec" to extend; each
    // sub-window was already sized to its own share of the narration in
    // twelvelabs.js). Leave these untouched here; they're handled in the
    // per-beat loop below instead.
    if (Array.isArray(sc.subWindows) && sc.subWindows.length > 1) return sc;
  ...
  // Emit sub-clips of ≤ maxClipSec for ONE window (forced visual transitions),
  // advancing the SHARED cursor/wrap state above. Extracted so multi-event
  // beats (see MULTI-EVENT below) can call this once per sub-window instead
  // of once per beat — for single-window beats this is byte-identical to the
  // original inline loop, just parameterized.
  function emitForWindow(sceneStart, sceneEnd, focus, needIn, beatIndex, reason) {
    let need = needIn;
    // Always jump the cursor to this window's start. Allowing backward jumps
    // is essential for non-linear films (flashbacks, parallel timelines)
    // where Claude's narrative ordering differs from strict movie
    // chronology. The old forward-only constraint caused beats whose scene
    // windows lay behind the current cursor to receive completely wrong
    // footage from wherever the cursor happened to be — the most common
    // cause of "no sync" complaints. Content accuracy beats diversity:
    // showing the right scene (possibly from a revisited position) is
    // always better than showing a unique but wrong scene.
    //
    // FIX (SYNC-FIXES #7): a CLIP-relocated window is [center - winHalf,
    // center + winHalf] with winHalf >= 8s. Always jumping to the window
    // START meant that when narration is shorter than winHalf (common for
    // short/importance-cut beats), the rendered footage was only
    // [center-8, center-8+need] — the matched moment at `center` itself was
    // never actually shown. If the CLIP/TEXT-MATCH pass stamped a focus,
    // center the take on it (clamped inside the window) instead of just
    // jumping to the start.
    if (Number.isFinite(focus) && Number.isFinite(sceneStart) && Number.isFinite(sceneEnd)) {
      // v3.0.7: place the GSPAN-matched moment ~30% into the beat (not 50%) so
      // the key action appears as the narration reaches it, not after describing it.
      const leadIn = Math.min(need * 0.30, Math.max(0, focus - sceneStart), Math.max(0, sceneEnd - sceneStart) * 0.5);
      cursor = Math.min(
        Math.max(sceneStart, focus - leadIn),
        Math.max(sceneStart, sceneEnd - need)
      );
    } else if (Number.isFinite(sceneStart)) {
      cursor = sceneStart;
    }
    ...
    // deferred to after each window's entire need is served.
    while (need >= minSegSec) {
      ...
      if (cursor >= ceiling - minSegSec) {
        ...
        console.warn(`[buildSyncedTimeline] wrap #${wrapCount} at beat ${beatIndex} → cursor reset to ${cursor.toFixed(1)}s (${(offset * 100).toFixed(0)}% of range)`);
      }
      ...
      const computedMaxCut = (Array.isArray(opts.beatImportances) || Array.isArray(opts.beatTypes))
        ? Math.max(MIN_CUT_SEC, computeMaxClipSec(
            Array.isArray(opts.beatImportances) ? opts.beatImportances[beatIndex] : undefined,
            Array.isArray(opts.beatTypes) ? opts.beatTypes[beatIndex] : undefined
          ))
        : maxClipSec;
      ...
      // narrower than the narration assigned to it — now common with
      // GSPAN's precise, often short windows — the loop silently kept
      // consuming footage PAST sceneEnd into whatever scene comes next
      // chronologically, producing completely unrelated footage (e.g.
      // "entering a room" narration showing a car driving because the next
      // scene's footage bled in). Clamp to sceneEnd too; when that's the
      // binding constraint (not the true end-of-source), stop instead of
      // overflowing.
      const sceneClampEnd = Number.isFinite(sceneEnd) ? Math.min(ceiling, sceneEnd) : ceiling;
      const clipEnd = Math.min(cursor + clipLen, sceneClampEnd);
      const taken = clipEnd - cursor;

      if (taken >= minSegSec) {
        _trackRange(cursor, clipEnd);
        pushSeg(out, cursor, clipEnd, beatIndex, reason, minSegSec);
        cursor = clipEnd;
        need -= taken;
      } else if (sceneClampEnd < ceiling - minSegSec) {
        // This window is exhausted before its share of the narration is — a
        // per-window footage shortfall, not whole-movie exhaustion. Stop
        // taking footage for this window rather than bleeding into the next
        // scene's unrelated content; the mux stage's tpad clone-frame tail
        // freezes the last frame for the remaining audio, which is always
        // closer to correct than showing the wrong scene.
        console.warn(`[buildSyncedTimeline] beat ${beatIndex}: window exhausted ${need.toFixed(1)}s short of narration — holding last frame (tpad) instead of overflowing into next scene`);
        need = 0;
        break;
      } else {
        // Not enough room before the true end-of-source ceiling — trigger wrap on next iteration.
        cursor = ceiling;
      }
    }
  }
  ...
  for (let i = 0; i < n; i++) {
    const need = Number(beatDurations[i]) || 0;
    if (need <= 0) continue;

    const reason = typeof workScenes[i].reason === "string" ? workScenes[i].reason : "";

    // MULTI-EVENT (2026-07-10): a beat with 2+ vision-verified sub-events
    // (see twelvelabs.js buildVisualQuery) carries `subWindows` — discrete,
    // possibly-distant clip locations that together depict the whole beat,
    // instead of one continuous window. Split this beat's narration time
    // evenly across them (v1: no proportional weighting) and emit each
    // through the SAME forced-cut inner loop as a single-window beat. The
    // default (single-element) case below is exactly the old behavior.
    const subWindows = Array.isArray(workScenes[i].subWindows) && workScenes[i].subWindows.length > 1
      ? workScenes[i].subWindows
      : [{ startSec: workScenes[i].startSec, endSec: workScenes[i].endSec, center: workScenes[i].focusSec }];

    const perWinNeed = need / subWindows.length;
    for (const win of subWindows) {
      emitForWindow(Number(win.startSec), Number(win.endSec), Number(win.center), perWinNeed, i, reason);
    }
  }
