 ...
 */
export async function buildVisualQuery(narration, anthropicApiKey, log) {
...
async function extractFrame(sourcePath, atSec, outPath) {
  await runFfmpeg(["-y", "-ss", Math.max(0, atSec).toFixed(3), "-i", sourcePath, "-frames:v", "1", "-q:v", "3", outPath]);
}
...
async function verifyCandidatesWithVision({
  candidates, sourcePath, query, anthropicApiKey, jobId, beatIndex, log, maxCandidates = 3, framesPerCandidate = 2, sceneContext = null,
}) {
  ...
  for (let i = 0; i < outScenes.length && attempted < maxBeats; i++) {
    ...
    try {
      ...
      const runOne = async (query, isVeryFirst) => {
        ...
        // Build scene context for vision verification (Upgrade 10)
      ...
      for (let e = 0; e < events.length; e++) {
        ...
        if (!r.ok) { lastRejection = r; continue; }
      ...
      if (kept.length === 0) {
        ...
        if (fallback) {
          ...
          beatLog[i].video_start = fallback.startSec;
          beatLog[i].video_end   = fallback.endSec;
          beatLog[i].clip_duration = fallback.endSec - fallback.startSec;
      ...
      beatLog[i].video_start = finalStart;
      beatLog[i].video_end   = finalEnd;
      beatLog[i].clip_duration = finalEnd - finalStart;
